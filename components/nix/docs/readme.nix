...
in
{
  perSystem.files.file."README.md".text = ''
    # infra
    ...
    nix build .#checks.${host.system}.cloud-kustomize \
      --no-link --accept-flake-config
    ```
    ...
    `${user.homeDirectory}/.config/sops/age/keys.txt` can decrypt every SOPS file
    ...
    `${user.homeDirectory}/Pictures/Wallpapers/current.png`. After replacing it,
    ...
    `components/nix/computers/${hostName}-disko.nix`. Verify that path first.
